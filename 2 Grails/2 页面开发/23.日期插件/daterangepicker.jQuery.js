...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text:'明天', dateStart:'tomorrow', dateEnd:'tomorrow' },
			{text:'最近3天', dateStart:'today-3days', dateEnd:'today' },
			{text:'最近一周', dateStart:'today-7days', dateEnd:'today' },
			{text:'最近一月', dateStart:'today-1month', dateEnd:'today' },
			{text:'最近一年', dateStart:'today-1year', dateEnd:'today' }
		], 
		...
		appendTo: '#date-select',
		onClose: function(){},
		onOpen: function(){
			//rp.find('.range-start').datepicker('option','maxDate',Date.parse('today'));
			//rp.find('.range-end').datepicker('option','maxDate',Date.parse('today'));
			},
		onChange: function(){},
	...
	var datepickerOptions = {
		onSelect: function() { 
				...
				if(rangeInput.length == 2){
					...
					if(rangeA>rangeB){
						rangeInput.val(rangeB+' '+ options.rangeSplitter +' '+rangeA);	
					}else{
	...
	jQuery.fn.clickActions = function(rp, rpPickers, doneBtn){
		...
		if(jQuery(this).is('.ui-daterangepicker-specificDate')){
			...
			rp.find('.range-start').restoreDateFromData().show(200);
			rp.find('.range-end').restoreDateFromData().hide(200);
			setTimeout(function(){doneBtn.fadeIn();}, 200);
		}
		else if(jQuery(this).is('.ui-daterangepicker-allDatesBefore')){
			doneBtn.hide();
			rpPickers.show();
			rp.find('.title-end').text( options.presets.allDatesBefore );
			rp.find('.range-start').saveDateToData().datepicker('setDate', options.earliestDate).hide(200);
			rp.find('.range-end').restoreDateFromData().show(200);
			setTimeout(function(){doneBtn.fadeIn();}, 200);
		}
			...
			rp.find('.range-start').restoreDateFromData().show(200);
			rp.find('.range-end').saveDateToData().datepicker('setDate', options.latestDate).hide(200);
			setTimeout(function(){doneBtn.fadeIn();}, 200);
		}
	...
	//picker divs
	var rpPickers = jQuery('<div class="ranges ui-widget-header ui-corner-all ui-helper-clearfix"><div class="range-start"><span class="title-start">开始日期</span></div><div class="range-end"><span class="title-end">结束日期</span></div></div>').appendTo(rp);
	rpPickers.find('.range-start, .range-end').datepicker(options.datepickerOptions);
	rpPickers.find('.range-start').datepicker('setDate', inputDateA);
	rpPickers.find('.range-end').datepicker('setDate', inputDateB);
	/*rpPickers.find('.range-start').click(function(){
		var range_end_start_date = 	fDate( rp.find('.range-start').datepicker('getDate'));
		rp.find('.range-end').datepicker('option','minDate',Date.parse(range_end_start_date));
	});
	rpPickers.find('.range-end').click(function(){
		var range_start_end_date = 	fDate( rp.find('.range-end').datepicker('getDate'));
		rp.find('.range-start').datepicker('option','maxDate',Date.parse(range_start_end_date));	
	});*/
	var doneBtn = jQuery('<button type="button" class="btnDone ui-state-default ui-corner-all">'+ options.doneButtonText +'</button>')
	.click(function(){
	...
	//hide em all
	rpPickers.css('display', '').find('.range-start, .range-end, .btnDone').css('display', ' ');
